...
export async function adjustPlayerBalance(roomCode, playerName, deltaEuro) {
  const normalizedRoomCode = normalizeRoomCode(roomCode);
  const normalizedPlayerName = String(playerName || "").trim();
  const parsedDelta = Number(deltaEuro);
  ...
  if (!normalizedPlayerName) {
    throw new Error("Seleziona un giocatore.");
  }
  ...
  if (!ledger) {
    throw new Error(`Il giocatore ${normalizedPlayerName} non appartiene a questa stanza.`);
  }

  const nextTotal = ledger.total + parsedDelta;
  if (nextTotal < ledger.locked) {
    throw new Error("Operazione non valida: il totale non puo scendere sotto i soldi gia in gioco.");
  }

  const nextAvailable = nextTotal - ledger.locked;

  const database = ensureDatabase();
  await update(ref(database, `rooms/${normalizedRoomCode}/players/${normalizedPlayerName}`), {
    total: nextTotal,
